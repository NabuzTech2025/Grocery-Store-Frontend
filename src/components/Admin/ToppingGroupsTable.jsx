...
import { getToppingGroups, updateToppingGroups, deleteToppingGroups, reactivateToppingGroups } from '@/api/AdminServices';
import Swal from 'sweetalert2';

const ToppingGroupsTable = ({ reload, onSuccess }) => {
  ...
  const fetchToppings = async () => {
    try {
      const res = await getToppingGroups(localStorage.getItem('store_id'));
      setData(res);
      setFilteredData(res);
      setLoading(false);
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await updateToppingGroups(currentToppingId, payload);
  ...
  const handleToggleDeleteRestore = async (item) => {
    ...
    const confirmText = isCurrentlyActive
      ? 'This will delete the topping Groups!'
      : 'This will restore the topping Groups!';
    ...
    try {
      if (isCurrentlyActive) {
        await deleteToppingGroups(item.id); // DELETE /toppings/:id
        onSuccess && onSuccess('Topping deleted!');
      } else {
        await reactivateToppingGroups(item.id); // PUT /toppings/:id/reactivate
...
export default ToppingGroupsTable;
